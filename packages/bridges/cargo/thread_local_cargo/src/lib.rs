// Auto-generated Cargo bridge for thread_local@1.1.9
...
fn type_registry() -> &'static Mutex<HashMap<String, HashMap<String, MethodFn>>> {
    ...
    // ── CachedThreadLocal<()> methods ──
    {
        let tn = "CachedThreadLocal".to_string();
        let mut methods: HashMap<String, MethodFn> = HashMap::new();
        methods.insert("get".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
            let this_ref = unsafe { &*(this_ptr as *const thread_local::CachedThreadLocal<()>) };
            let __result = this_ref.get();
        ...
        methods.insert("clear".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
            let this_ref = unsafe { &mut *(this_ptr as *mut thread_local::CachedThreadLocal<()>) };
            let __result = this_ref.clear();
            serde_json::to_value(&__result).map_err(|e| e.to_string())
        });
        methods.insert("get_or_default".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
            let this_ref = unsafe { &*(this_ptr as *const thread_local::CachedThreadLocal<()>) };
            let __result = this_ref.get_or_default();
            serde_json::to_value(&__result).map_err(|e| e.to_string())
        });
    ...
    // ── ThreadLocal<()> methods ──
    {
        let tn = "ThreadLocal".to_string();
        let mut methods: HashMap<String, MethodFn> = HashMap::new();
        methods.insert("get".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
            let this_ref = unsafe { &*(this_ptr as *const thread_local::ThreadLocal<()>) };
            let __result = this_ref.get();
            serde_json::to_value(&__result).map_err(|e| e.to_string())
        });
        methods.insert("clear".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
            let this_ref = unsafe { &mut *(this_ptr as *mut thread_local::ThreadLocal<()>) };
            let __result = this_ref.clear();
            serde_json::to_value(&__result).map_err(|e| e.to_string())
        });
        methods.insert("get_or_default".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
...
fn ctor_registry() -> &'static Mutex<HashMap<String, HashMap<String, CtorFn>>> {
    ...
    // ── CachedThreadLocal<()> constructors ──
    {
        let tn = "CachedThreadLocal".to_string();
        ...
        ctors.insert("new".to_string(), |_args: Vec<serde_json::Value>| -> std::result::Result<*mut c_void, String> {
            let __val = thread_local::CachedThreadLocal::<()>::new();
            Ok(Box::into_raw(Box::new(__val)) as *mut c_void)
        });
        ctor_map.insert(tn, ctors);
    }
    // ── ThreadLocal<()> constructors ──
    {
        let tn = "ThreadLocal".to_string();
        let mut ctors: HashMap<String, CtorFn> = HashMap::new();
        ctors.insert("with_capacity".to_string(), |args: Vec<serde_json::Value>| -> std::result::Result<*mut c_void, String> {
            ...
            let a0 = __deser(a0)?;
            let __val = thread_local::ThreadLocal::<()>::with_capacity(a0);
            Ok(Box::into_raw(Box::new(__val)) as *mut c_void)
        });
        ctor_map.insert(tn, ctors);
    }
...
fn drop_registry() -> &'static Mutex<HashMap<String, DropFn>> {
    ...
    m.insert("CachedThreadLocal".to_string(), {
        let f: DropFn = |ptr| unsafe { drop(Box::from_raw(ptr as *mut thread_local::CachedThreadLocal<()>)); };
        f
    });
    m.insert("ThreadLocal".to_string(), {
        let f: DropFn = |ptr| unsafe { drop(Box::from_raw(ptr as *mut thread_local::ThreadLocal<()>)); };
        f
    });
        Mutex::new(m)
    })
...
#[no_mangle]
pub extern "C" fn thread_local_import() -> *mut c_void {
    registry(); // ensure initialized
    store(serde_json::Value::Object(serde_json::Map::new()))
}

#[no_mangle]
pub extern "C" fn thread_local_free(ptr: *mut c_void) {
    if !ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn thread_local_free_cstr(ptr: *mut std::ffi::c_char) {
    if !ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn thread_local_str(s: *const std::ffi::c_char) -> *mut c_void {
    let s = unsafe { CStr::from_ptr(s) }.to_string_lossy().to_string();
    store(serde_json::Value::String(s))
}

#[no_mangle]
pub extern "C" fn thread_local_int(v: i64) -> *mut c_void {
    store(serde_json::Value::Number(v.into()))
}

#[no_mangle]
pub extern "C" fn thread_local_float(v: f64) -> *mut c_void {
    store(serde_json::Value::Number(
        serde_json::Number::from_f64(v).unwrap_or(serde_json::Number::from(0))))
}

#[no_mangle]
pub extern "C" fn thread_local_tuple(items: *mut *mut c_void, count: i32) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn thread_local_list(items: *mut *mut c_void, count: i32) -> *mut c_void {
    let mut vec = Vec::with_capacity(count as usize);
    for i in 0..count {
        let ptr = unsafe { *items.offset(i as isize) };
        if ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn thread_local_tuple2(a: *mut c_void, b: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
...
#[no_mangle]
pub extern "C" fn thread_local_tuple3(a: *mut c_void, b: *mut c_void, c: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
...
#[no_mangle]
pub extern "C" fn thread_local_tuple4(a: *mut c_void, b: *mut c_void, c: *mut c_void, d: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn thread_local_list2(a: *mut c_void, b: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
    ]))
...
#[no_mangle]
pub extern "C" fn thread_local_list3(a: *mut c_void, b: *mut c_void, c: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn thread_local_list4(a: *mut c_void, b: *mut c_void, c: *mut c_void, d: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn thread_local_dict() -> *mut c_void {
    store(serde_json::Value::Object(serde_json::Map::new()))
}

#[no_mangle]
pub extern "C" fn thread_local_dict_set(d: *mut c_void, key: *const std::ffi::c_char, val: *mut c_void) -> i32 {
...
#[no_mangle]
pub extern "C" fn thread_local_to_cstr(obj: *mut c_void) -> *mut std::ffi::c_char {
...
#[no_mangle]
pub extern "C" fn thread_local_getattr(obj: *mut c_void, name: *const std::ffi::c_char) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn thread_local_call(
    _handle: *mut c_void,
...
#[no_mangle]
pub extern "C" fn thread_local_call1(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
    a: *mut c_void,
) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn thread_local_call2(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn thread_local_call3(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn thread_local_call4(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn thread_local_call5(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn thread_local_call6(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn thread_local_call_kw(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
    args: *mut c_void,
    kwargs: *mut c_void,
...
#[no_mangle]
pub extern "C" fn thread_local_construct(
...
#[no_mangle]
pub extern "C" fn thread_local_call_method(
    handle: *mut c_void,
    type_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn thread_local_free_type(handle: *mut c_void, type_name: *const std::ffi::c_char) {
...
#[no_mangle]
pub extern "C" fn thread_local_last_error() -> *mut std::ffi::c_char {
    LAST_ERROR.with(|e| {
