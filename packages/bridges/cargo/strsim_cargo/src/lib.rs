// Auto-generated Cargo bridge for strsim@0.11.1
...
fn registry() -> &'static Mutex<HashMap<String, RustFn>> {
    ...
    REG.get_or_init(|| {
        let mut m: HashMap<String, RustFn> = HashMap::new();
        m.insert("jaro".to_string(), |args| {
            ...
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::jaro(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
        m.insert("jaro_winkler".to_string(), |args| {
            let a0: serde_json::Value = args.get(0).ok_or_else(|| "0: missing arg".to_string())?.clone();
            let a0_owned: String = __deser(a0)?;
            let a0 = &a0_owned;
            let a1: serde_json::Value = args.get(1).ok_or_else(|| "1: missing arg".to_string())?.clone();
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::jaro_winkler(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
        m.insert("levenshtein".to_string(), |args| {
            let a0: serde_json::Value = args.get(0).ok_or_else(|| "0: missing arg".to_string())?.clone();
            let a0_owned: String = __deser(a0)?;
            let a0 = &a0_owned;
            let a1: serde_json::Value = args.get(1).ok_or_else(|| "1: missing arg".to_string())?.clone();
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::levenshtein(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
        m.insert("normalized_levenshtein".to_string(), |args| {
            let a0: serde_json::Value = args.get(0).ok_or_else(|| "0: missing arg".to_string())?.clone();
            let a0_owned: String = __deser(a0)?;
            let a0 = &a0_owned;
            let a1: serde_json::Value = args.get(1).ok_or_else(|| "1: missing arg".to_string())?.clone();
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::normalized_levenshtein(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
        m.insert("osa_distance".to_string(), |args| {
            let a0: serde_json::Value = args.get(0).ok_or_else(|| "0: missing arg".to_string())?.clone();
            let a0_owned: String = __deser(a0)?;
            let a0 = &a0_owned;
            let a1: serde_json::Value = args.get(1).ok_or_else(|| "1: missing arg".to_string())?.clone();
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::osa_distance(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
        m.insert("damerau_levenshtein".to_string(), |args| {
            let a0: serde_json::Value = args.get(0).ok_or_else(|| "0: missing arg".to_string())?.clone();
            let a0_owned: String = __deser(a0)?;
            let a0 = &a0_owned;
            let a1: serde_json::Value = args.get(1).ok_or_else(|| "1: missing arg".to_string())?.clone();
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::damerau_levenshtein(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
        m.insert("normalized_damerau_levenshtein".to_string(), |args| {
            let a0: serde_json::Value = args.get(0).ok_or_else(|| "0: missing arg".to_string())?.clone();
            let a0_owned: String = __deser(a0)?;
            let a0 = &a0_owned;
            let a1: serde_json::Value = args.get(1).ok_or_else(|| "1: missing arg".to_string())?.clone();
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::normalized_damerau_levenshtein(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
        m.insert("sorensen_dice".to_string(), |args| {
            let a0: serde_json::Value = args.get(0).ok_or_else(|| "0: missing arg".to_string())?.clone();
            let a0_owned: String = __deser(a0)?;
            let a0 = &a0_owned;
            let a1: serde_json::Value = args.get(1).ok_or_else(|| "1: missing arg".to_string())?.clone();
            let a1_owned: String = __deser(a1)?;
            let a1 = &a1_owned;
            let result = strsim::sorensen_dice(a0, a1);
            Ok(serde_json::to_value(result).map_err(|e| e.to_string())?)
        });
...
#[no_mangle]
pub extern "C" fn strsim_import() -> *mut c_void {
    registry(); // ensure initialized
    store(serde_json::Value::Object(serde_json::Map::new()))
}

#[no_mangle]
pub extern "C" fn strsim_free(ptr: *mut c_void) {
    if !ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn strsim_free_cstr(ptr: *mut std::ffi::c_char) {
    if !ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn strsim_str(s: *const std::ffi::c_char) -> *mut c_void {
    let s = unsafe { CStr::from_ptr(s) }.to_string_lossy().to_string();
    store(serde_json::Value::String(s))
}

#[no_mangle]
pub extern "C" fn strsim_int(v: i64) -> *mut c_void {
    store(serde_json::Value::Number(v.into()))
}

#[no_mangle]
pub extern "C" fn strsim_float(v: f64) -> *mut c_void {
    store(serde_json::Value::Number(
        serde_json::Number::from_f64(v).unwrap_or(serde_json::Number::from(0))))
}

#[no_mangle]
pub extern "C" fn strsim_tuple(items: *mut *mut c_void, count: i32) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn strsim_list(items: *mut *mut c_void, count: i32) -> *mut c_void {
    let mut vec = Vec::with_capacity(count as usize);
    for i in 0..count {
        let ptr = unsafe { *items.offset(i as isize) };
        if ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn strsim_tuple2(a: *mut c_void, b: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
...
#[no_mangle]
pub extern "C" fn strsim_tuple3(a: *mut c_void, b: *mut c_void, c: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
...
#[no_mangle]
pub extern "C" fn strsim_tuple4(a: *mut c_void, b: *mut c_void, c: *mut c_void, d: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn strsim_list2(a: *mut c_void, b: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
    ]))
...
#[no_mangle]
pub extern "C" fn strsim_list3(a: *mut c_void, b: *mut c_void, c: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn strsim_list4(a: *mut c_void, b: *mut c_void, c: *mut c_void, d: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn strsim_dict() -> *mut c_void {
    store(serde_json::Value::Object(serde_json::Map::new()))
}

#[no_mangle]
pub extern "C" fn strsim_dict_set(d: *mut c_void, key: *const std::ffi::c_char, val: *mut c_void) -> i32 {
...
#[no_mangle]
pub extern "C" fn strsim_to_cstr(obj: *mut c_void) -> *mut std::ffi::c_char {
...
#[no_mangle]
pub extern "C" fn strsim_getattr(obj: *mut c_void, name: *const std::ffi::c_char) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn strsim_call_kw(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
    args: *mut c_void,
    kwargs: *mut c_void,
...
#[no_mangle]
pub extern "C" fn strsim_construct(
...
#[no_mangle]
pub extern "C" fn strsim_call_method(
    handle: *mut c_void,
    type_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn strsim_free_type(handle: *mut c_void, type_name: *const std::ffi::c_char) {
...
#[no_mangle]
pub extern "C" fn strsim_last_error() -> *mut std::ffi::c_char {
    LAST_ERROR.with(|e| {
