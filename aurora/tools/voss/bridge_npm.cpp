...
void gen_quickjs_npm_wrapper(const std::string& pkg, const std::string& dir)
{
    ...
    /* Generate the C bridge DLL using embedded QuickJS engine */
    {
...
#else
        cw << "/* ── TLS via OpenSSL (dynamically loaded on POSIX) ── */\n";
        cw << "#include <dlfcn.h>\n";
        cw << "#define MAX_TLS_SESS 32\n";
        cw << "/* OpenSSL function typedefs */\n";
        cw << "typedef void* (*pSSL_new_t)(void*);\n";
        cw << "typedef void (*pSSL_free_t)(void*);\n";
        cw << "typedef int (*pSSL_set_fd_t)(void*,int);\n";
        cw << "typedef int (*pSSL_connect_t)(void*);\n";
        cw << "typedef int (*pSSL_read_t)(void*,void*,int);\n";
        cw << "typedef int (*pSSL_write_t)(void*,const void*,int);\n";
        cw << "typedef int (*pSSL_shutdown_t)(void*);\n";
        cw << "typedef void* (*pSSL_CTX_new_t)(void*);\n";
        cw << "typedef void (*pSSL_CTX_free_t)(void*);\n";
        cw << "typedef void* (*pTLS_client_method_t)(void);\n";
        cw << "typedef long (*pSSL_ctrl_t)(void*,int,long,void*);\n";
        cw << "typedef int (*pSSL_get_error_t)(void*,int);\n";
        cw << "static pSSL_new_t pSSL_new=NULL;\n";
        cw << "static pSSL_free_t pSSL_free=NULL;\n";
        cw << "static pSSL_set_fd_t pSSL_set_fd=NULL;\n";
        cw << "static pSSL_connect_t pSSL_connect=NULL;\n";
        cw << "static pSSL_read_t pSSL_read=NULL;\n";
        cw << "static pSSL_write_t pSSL_write=NULL;\n";
        cw << "static pSSL_shutdown_t pSSL_shutdown=NULL;\n";
        cw << "static pSSL_CTX_new_t pSSL_CTX_new=NULL;\n";
        cw << "static pSSL_CTX_free_t pSSL_CTX_free=NULL;\n";
        cw << "static pTLS_client_method_t pTLS_client_method=NULL;\n";
        cw << "static pSSL_ctrl_t pSSL_ctrl=NULL;\n";
        cw << "static pSSL_get_error_t pSSL_get_error=NULL;\n";
        cw << "static void* g_ssl_ctx=NULL;\n";
        cw << "static int tls_init(void){\n";
        cw << "  static int n=0;if(n)return n;\n";
        cw << "  void*h=dlopen(\"libssl.so.3\",RTLD_LAZY|RTLD_LOCAL);\n";
        cw << "  if(!h)h=dlopen(\"libssl.so.1.1\",RTLD_LAZY|RTLD_LOCAL);\n";
        cw << "  if(!h)h=dlopen(\"libssl.so\",RTLD_LAZY|RTLD_LOCAL);\n";
        cw << "  if(!h){n=-1;return n;}\n";
        cw << "  pSSL_new=(void*)dlsym(h,\"SSL_new\");\n";
        cw << "  pSSL_free=(void*)dlsym(h,\"SSL_free\");\n";
        cw << "  pSSL_set_fd=(void*)dlsym(h,\"SSL_set_fd\");\n";
        cw << "  pSSL_connect=(void*)dlsym(h,\"SSL_connect\");\n";
        cw << "  pSSL_read=(void*)dlsym(h,\"SSL_read\");\n";
        cw << "  pSSL_write=(void*)dlsym(h,\"SSL_write\");\n";
        cw << "  pSSL_shutdown=(void*)dlsym(h,\"SSL_shutdown\");\n";
        cw << "  pSSL_CTX_new=(void*)dlsym(h,\"SSL_CTX_new\");\n";
        cw << "  pSSL_CTX_free=(void*)dlsym(h,\"SSL_CTX_free\");\n";
        cw << "  pTLS_client_method=(void*)dlsym(h,\"TLS_client_method\");\n";
        cw << "  pSSL_ctrl=(void*)dlsym(h,\"SSL_CTX_ctrl\");\n";
        cw << "  pSSL_get_error=(void*)dlsym(h,\"SSL_get_error\");\n";
        cw << "  if(!pSSL_new||!pSSL_free||!pSSL_set_fd||!pSSL_connect||!pSSL_read||!pSSL_write||!pSSL_shutdown||!pSSL_CTX_new||!pSSL_CTX_free||!pTLS_client_method||!pSSL_ctrl||!pSSL_get_error){n=-1;return n;}\n";
        cw << "  void*m=pTLS_client_method();if(!m){n=-1;return n;}\n";
        cw << "  g_ssl_ctx=pSSL_CTX_new(m);if(!g_ssl_ctx){n=-1;return n;}\n";
        cw << "  pSSL_ctrl(g_ssl_ctx,0x20,0x300,NULL); /* SSL_CTRL_OPTIONS, SSL_OP_NO_SSLv2|SSL_OP_NO_SSLv3 */\n";
        cw << "  n=1;return 1;}\n";
        cw << "  static struct tls_ses{int fd;void*ssl;int ok;}g_tls_ses[MAX_TLS_SESS];\n";
        cw << "static int tls_handshake(int fd,void**ssl_out){\n";
        cw << "  if(tls_init()<0)return-10;\n";
        cw << "  void*ssl=pSSL_new(g_ssl_ctx);if(!ssl)return-1;\n";
        cw << "  pSSL_set_fd(ssl,fd);\n";
        cw << "  int r=pSSL_connect(ssl);\n";
        cw << "  if(r<=0){int e=pSSL_get_error(ssl,r);pSSL_free(ssl);return-2-e;}\n";
        cw << "  *ssl_out=ssl;return 0;}\n";
        cw << "static JSValue bridge_tls_connect(JSContext*ctx,JSValueConst t,int ac,JSValueConst*av){\n";
        cw << "  if(ac<2)return JS_ThrowTypeError(ctx,\"tls_connect:need host port\");\n";
        cw << "  int32_t port;JS_ToInt32(ctx,&port,av[1]);if(port<1)return JS_ThrowTypeError(ctx,\"bad port\");\n";
        cw << "  JSValue fdv=bridge_net_connect(ctx,t,ac,av);\n";
        ...
        cw << "  memset(&g_tls_ses[slot],0,sizeof(g_tls_ses[slot]));\n";
        cw << "  g_tls_ses[slot].fd=fd;\n";
        cw << "  int r=tls_handshake(fd,&g_tls_ses[slot].ssl);\n";
        cw << "  if(r<0){memset(&g_tls_ses[slot],0,sizeof(g_tls_ses[slot]));pclosesocket(fd);return JS_ThrowTypeError(ctx,\"tls:handshake failed\");}\n";
        cw << "  g_tls_ses[slot].ok=1;return JS_NewInt32(ctx,slot+1);}\n\n";
        cw << "static JSValue bridge_tls_write(JSContext*ctx,JSValueConst t,int ac,JSValueConst*av){\n";
        cw << "  if(ac<2)return JS_ThrowTypeError(ctx,\"tls_write:need handle data\");\n";
        cw << "  int32_t h;JS_ToInt32(ctx,&h,av[0]);if(h<1||h>MAX_TLS_SESS||!g_tls_ses[h-1].ok)return JS_ThrowTypeError(ctx,\"bad handle\");\n";
        cw << "  const char*data=JS_ToCString(ctx,av[1]);if(!data)return JS_ThrowTypeError(ctx,\"bad data\");\n";
        cw << "  int n=pSSL_write(g_tls_ses[h-1].ssl,data,(int)strlen(data));\n";
        cw << "  JS_FreeCString(ctx,data);\n";
        cw << "  if(n<0)return JS_ThrowTypeError(ctx,\"SSL_write failed\");return JS_NewInt32(ctx,n);}\n\n";
        cw << "static JSValue bridge_tls_read(JSContext*ctx,JSValueConst t,int ac,JSValueConst*av){\n";
        cw << "  if(ac<1)return JS_ThrowTypeError(ctx,\"tls_read:need handle\");\n";
        cw << "  int32_t h;JS_ToInt32(ctx,&h,av[0]);if(h<1||h>MAX_TLS_SESS||!g_tls_ses[h-1].ok)return JS_ThrowTypeError(ctx,\"bad handle\");\n";
        cw << "  char ibuf[16384];int n=pSSL_read(g_tls_ses[h-1].ssl,ibuf,(int)sizeof(ibuf)-1);\n";
        cw << "  if(n<=0){if(n==0)return JS_NULL;return JS_ThrowTypeError(ctx,\"SSL_read failed\");}\n";
        cw << "  ibuf[n]=0;return JS_NewString(ctx,ibuf);}\n\n";
        cw << "static JSValue bridge_tls_close(JSContext*ctx,JSValueConst t,int ac,JSValueConst*av){\n";
        cw << "  if(ac<1)return JS_FALSE;int32_t h;JS_ToInt32(ctx,&h,av[0]);\n";
        cw << "  if(h<1||h>MAX_TLS_SESS||!g_tls_ses[h-1].ok)return JS_FALSE;\n";
        cw << "  struct tls_ses*ss=&g_tls_ses[h-1];\n";
        cw << "  pSSL_shutdown(ss->ssl);pSSL_free(ss->ssl);\n";
        cw << "  pclosesocket(ss->fd);memset(ss,0,sizeof(*ss));return JS_TRUE;}\n\n";
#endif
