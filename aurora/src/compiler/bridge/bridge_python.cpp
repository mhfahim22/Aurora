...
void Codegen::gen_bridge_python_fn(const ASTNode* node) {
    if (!node) return;
    ...
    /* ── Emit wrapper function that calls aurora_bridge_python_call ── */
    auto* ptr_type = llvm::PointerType::get(ctx_, 0);

    /* Create a global function pointer for the bridge runtime function */
    std::string bridge_fn_name = "aurora_bridge_python_call";
    auto* bridge_fn_type = llvm::FunctionType::get(
        ptr_type,                       /* returns void* */
        { ptr_type,                     /* module_name: cstring */
          ptr_type,                     /* func_name: cstring */
          ptr_type,                     /* args: void** array */
          llvm::Type::getInt32Ty(ctx_), /* arg_count: i32 */
          ptr_type },                   /* ret_type_name: cstring */
        false);
    ...
    /* ── Create the extern wrapper function with proper Aurora signature ── */
    ...
    /* Create entry block */
    auto* entry = llvm::BasicBlock::Create(ctx_, "py.entry", wrapper);
    builder_->SetInsertPoint(entry);

    /* Build the args array: allocate void*[arg_count] on stack */
    ...
    if (arg_count > 0) {
        ...
        args_array = builder_->CreateAlloca(arr_type, nullptr, "py.args_arr");
        ...
        for (auto& arg : wrapper->args()) {
            llvm::Value* arg_ptr = builder_->CreateGEP(
                ...
                "py.arg." + std::to_string(idx));
            /* Convert i64 to ptr (bitcast) if needed */
            llvm::Value* arg_val = &arg;
            if (arg.getType()->isIntegerTy() && arg.getType()->getIntegerBitWidth() == 64) {
                arg_val = builder_->CreateIntToPtr(arg_val, ptr_type, "py.arg.ptr." + std::to_string(idx));
            } else {
                arg_val = builder_->CreateBitCast(
                    builder_->CreateAlloca(arg.getType(), nullptr, "py.arg.copy." + std::to_string(idx)),
                    ptr_type, "py.arg.ptr." + std::to_string(idx));
            }
    ...
    llvm::Value* mod_name_str = builder_->CreateGlobalStringPtr(mod_name, "py.mod");
    llvm::Value* fn_name_str = builder_->CreateGlobalStringPtr(fname, "py.fn");
    llvm::Value* arg_count_val = llvm::ConstantInt::get(ctx_, llvm::APInt(32, arg_count));
    llvm::Value* ret_type_str = builder_->CreateGlobalStringPtr(ret_type_name, "py.ret");
    ...
    llvm::Value* result = builder_->CreateCall(bridge_fn, call_args, "py.result");

    /* Convert returned void* back to the expected type */
    if (!ret_type->isVoidTy()) {
        if (ret_type->isIntegerTy() && ret_type->getIntegerBitWidth() == 64) {
            result = builder_->CreatePtrToInt(result, ret_type, "py.result.int");
        } else if (ret_type->isDoubleTy()) {
            /* Bitcast through i64 */
            result = builder_->CreatePtrToInt(result, i64_ty(), "py.result.i64");
            result = builder_->CreateBitCast(result, ret_type, "py.result.f64");
        } else {
            result = builder_->CreateBitCast(result, ret_type, "py.result.bitcast");
        }
    }
    ...
    std::cerr << "[bridge:python] extern function '" << fname
