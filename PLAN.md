...
|------|------|----------|--------|-------|
| Struct declaration & literal | Compiler | High | ✅ | Parser (`parse_struct.cpp`) → TypeChecker (`register_struct()`) → Codegen (LLVM struct type, GEP field access, alloca). **Tested**: `test_struct_decl.aufra` PASS |
| Enum declaration | Compiler | High | ✅ | Parser (`parse_enum.cpp`) → TypeChecker (`register_enum()`, variants as i64). Codegen: no-op (enums are i64). **Tested**: `test_enum_decl.aufra` PASS |
| Interface declaration | Compiler | High | ✅ | Parser (`parse_interface.cpp`, supports `extends`) → TypeChecker (`register_interface()`, `validate_interface_impl()`). Codegen: compile-time only. **Tested**: `test_interface_decl.aufra` PASS |
| Type alias | Compiler | High | ✅ | Parser (`parse_type_alias.cpp`) → TypeChecker (`register_type_alias()`, recursive resolution). Codegen: compile-time only. **Tested**: `test_typealias.aufra` PASS |
| Match/switch expressions | Compiler | High | ✅ | Full pattern matching: integers, variable binding, array destructuring, struct patterns, wildcard (`_`). Parser → TypeChecker (pattern variable binding) → Codegen (LLVM IR with basic blocks, fallthrough). **Tested**: `test_match_comprehensive.aufra` PASS, `test_match.aufra` PASS, `test_switch.aufra` PASS |
| Class OOP (inheritance, abstract, final, visibility, interfaces) | Compiler | High | ✅ | Full: `parse_class.cpp` → `typechecker_oop.cpp` → `codegen_oop.cpp` (vtables, field access, method dispatch). **Tested**: `test_abstract.aufra`, `test_encap_pass.aufra`, `test_encapsulation.aufra`, `test_methods.aufra`, `test_methods2.aufra`, `test_methods3.aufra`, `test_polymorphism.aufra` all PASS |
...
| Collection utility functions | Compiler | High | ✅ | 24 `list_*`/`map_*`/`set_*`/`stack_*`/`queue_*`/`vector_*`/`json_*` functions registered in typechecker + codegen; `json_get` LLVM declaration added. **Tested**: `test_std_collections.aufra` PASS |
...
| Pure-Aurora std lib (libc/ directory) | Std Lib | Medium | ✅ | `libc/collections.auf`, `libc/string.auf`, `libc/math.auf` created; importable via `import "collections"` or `import libc:collections`. Verified with `test_import.aufra` |
| Full test suite | Tests | High | ✅ | **38/38 core tests PASS** (including all new Phase 1 tests) |

**Verification**: All 38 `.aufra` tests under `aurora/tests/core/` compiled and ran successfully via `aurorac.exe --emit-obj` → link → execute.
...
| Event/signal/callback runtime | Runtime | Medium | ✅ Dedicated `AuroraEventBus` runtime in `event_bus.cpp` with `aurora_event_on/off/emit`; `builtin_event_on/off/emit` wrappers; `gen_signal`/`gen_emit` rewritten to call event bus; tested with `test_event_fiber.aufra` |
| Fiber/coroutine support | Runtime | Medium | ✅ `AuroraFiber` runtime in `fiber.cpp` with `aurora_fiber_create/destroy/yield/resume/is_done/get_result`; builtin wrappers; tested with `test_event_fiber.aufra` |
...
| Cross-platform GUI backend (Win32 vs X11 vs Cocoa) | Runtime/UI | Medium | ✅ | `gui.cpp` has Win32 + X11 backends; `gui_mac.mm` provides full Cocoa backend (NSWindow, NSButton, NSTextField, NSTableView); CMakeLists.txt compiles per-platform; `test_gui_crossplatform.aufra` tests all backends |
...
| Task | Area | Priority | Notes |
|------|------|----------|-------|
| Compiler/parser test coverage | Tests | High | ✅ Created 7 `.aufra` test files covering full language surface: `lexer/test_lexer_keywords.aufra` (all keyword tokenization), `lexer/test_lexer_comprehensive.aufra` (operators, bitwise, comparisons, attributes, strings, complex expressions, multi-line, block comments, identifiers), `parser/test_parser_expressions.aufra` (arithmetic precedence, comparison, boolean, bitwise, mixed), `parser/test_parser_statements.aufra` (if/elif/else, while, for, recursion, closures, braces, switch), `parser/test_parser_classes.aufra` (classes, inheritance, abstract, encapsulation, interfaces, structs, enums, type aliases), `parser/test_parser_comprehensive.aufra` (nested if, nested loops, function args, recursion, arrays, break/continue, lambdas, closures, brace blocks), `parser/test_parser_advanced.aufra` (polymorphism, match, pointers, references, async, try/throw) |
| Cross-platform CI (macOS builds) | CI | Medium | ✅ Added `build-macos` job to `.github/workflows/build.yml` — installs LLVM 19 via Homebrew, builds core targets (aurorac, aurora_runtime, voss, aurora_lsp), builds and runs Phase 8 tests (test_fuzz_parser, bench_compiler), validates all `.aufra` test files via `run_all_tests.ps1`, runs npm bridge cross-compile test via gcc |
| Fuzz testing for parser | Tests | Medium | ✅ Created `test_fuzz_parser.cpp` — generates random `.aufra` code snippets from a pool of 20+ statement templates, modifiers, operators, and keywords; compiles each through `aurorac.exe` subprocess; reports crashes/throughput; built as `test_fuzz_parser` CMake target; verified 100 runs at 35 inputs/sec, 0 crashes |
| Memory management stress tests | Tests | Medium | ✅ Created 5 `.aufra` stress tests: `Stress/stress_memory_arena.aufra` (arena churn, nested, deep chain), `Stress/stress_memory_gc.aufra` (GC cycle chains, many objects, nested cycles, interleaved), `Stress/stress_memory_arc.aufra` (ARC share chains, cross-function, shared update, multi-share), `Stress/stress_memory_raii.aufra` (RAII chain, nested scope, early return, bulk), `Stress/stress_large_array.aufra` (100K-element array, nested 1000×100, deep recursion array) |
| UI/game/backend integration tests | Tests | Low | ✅ Created `domains/test_domains_backend_full.aufra` (server lifecycle, middleware, cache, session, auth, CORS, DB, rate limit, route group, stream, metrics, events, serialize), `domains/test_domains_game_full.aufra` (entity lifecycle, velocity, sprite, camera, physics, audio, animation, input, update, object), `domains/test_domains_ui_full.aufra` (component lifecycle, tree, style, theme, route, layout, animation, page, nested render, state updates) |
...
| Examples: more real-world programs | Docs | Low | ✅ Created 5 new example programs: web_server.aufra (full backend with routes, middleware, sessions, auth, DB), game_demo.aufra (space shooter with entities, physics, input, collision), async_demo.aufra (async/await, spawn, channels, fibers, event bus), data_processing.aufra (CSV loading, filter, sort, map, reduce, JSON), oop_comprehensive.aufra (interfaces, abstract, polymorphism, structs, enums, encapsulation) |
...
| `voss test` / `voss bench` commands | Tools | Low | ✅ `voss test` discovers `tests/*.aufra` files, compiles each with `aurorac`, runs them, reports pass/fail with timing. `voss bench [pkgs]` compiles entry point, runs 10 iterations, reports average execution time in ms |
