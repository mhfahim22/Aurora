// Auto-generated Cargo bridge for parking_lot@0.12.5
...
fn type_registry() -> &'static Mutex<HashMap<String, HashMap<String, MethodFn>>> {
    ...
    // ── Condvar methods ──
    {
        let tn = "Condvar".to_string();
        let mut methods: HashMap<String, MethodFn> = HashMap::new();
        methods.insert("notify_one".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
            let this_ref = unsafe { &*(this_ptr as *const parking_lot::Condvar) };
            let __result = this_ref.notify_one();
            serde_json::to_value(&__result).map_err(|e| e.to_string())
        });
        methods.insert("notify_all".to_string(), |this_ptr: *mut c_void, _args: Vec<serde_json::Value>| -> std::result::Result<serde_json::Value, String> {
            let this_ref = unsafe { &*(this_ptr as *const parking_lot::Condvar) };
            let __result = this_ref.notify_all();
            serde_json::to_value(&__result).map_err(|e| e.to_string())
        });
...
fn drop_registry() -> &'static Mutex<HashMap<String, DropFn>> {
    ...
    m.insert("Condvar".to_string(), {
        let f: DropFn = |ptr| unsafe { drop(Box::from_raw(ptr as *mut parking_lot::Condvar)); };
        f
    });
        Mutex::new(m)
    })
...
#[no_mangle]
pub extern "C" fn parking_lot_import() -> *mut c_void {
    registry(); // ensure initialized
    store(serde_json::Value::Object(serde_json::Map::new()))
}

#[no_mangle]
pub extern "C" fn parking_lot_free(ptr: *mut c_void) {
    if !ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn parking_lot_free_cstr(ptr: *mut std::ffi::c_char) {
    if !ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn parking_lot_str(s: *const std::ffi::c_char) -> *mut c_void {
    let s = unsafe { CStr::from_ptr(s) }.to_string_lossy().to_string();
    store(serde_json::Value::String(s))
}

#[no_mangle]
pub extern "C" fn parking_lot_int(v: i64) -> *mut c_void {
    store(serde_json::Value::Number(v.into()))
}

#[no_mangle]
pub extern "C" fn parking_lot_float(v: f64) -> *mut c_void {
    store(serde_json::Value::Number(
        serde_json::Number::from_f64(v).unwrap_or(serde_json::Number::from(0))))
}

#[no_mangle]
pub extern "C" fn parking_lot_tuple(items: *mut *mut c_void, count: i32) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn parking_lot_list(items: *mut *mut c_void, count: i32) -> *mut c_void {
    let mut vec = Vec::with_capacity(count as usize);
    for i in 0..count {
        let ptr = unsafe { *items.offset(i as isize) };
        if ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn parking_lot_tuple2(a: *mut c_void, b: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
...
#[no_mangle]
pub extern "C" fn parking_lot_tuple3(a: *mut c_void, b: *mut c_void, c: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
...
#[no_mangle]
pub extern "C" fn parking_lot_tuple4(a: *mut c_void, b: *mut c_void, c: *mut c_void, d: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn parking_lot_list2(a: *mut c_void, b: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
    ]))
...
#[no_mangle]
pub extern "C" fn parking_lot_list3(a: *mut c_void, b: *mut c_void, c: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn parking_lot_list4(a: *mut c_void, b: *mut c_void, c: *mut c_void, d: *mut c_void) -> *mut c_void {
    store(serde_json::Value::Array(vec![
        if a.is_null() { serde_json::Value::Null } else { unsafe { retrieve(a) }.clone() },
        if b.is_null() { serde_json::Value::Null } else { unsafe { retrieve(b) }.clone() },
        if c.is_null() { serde_json::Value::Null } else { unsafe { retrieve(c) }.clone() },
...
#[no_mangle]
pub extern "C" fn parking_lot_dict() -> *mut c_void {
    store(serde_json::Value::Object(serde_json::Map::new()))
}

#[no_mangle]
pub extern "C" fn parking_lot_dict_set(d: *mut c_void, key: *const std::ffi::c_char, val: *mut c_void) -> i32 {
...
#[no_mangle]
pub extern "C" fn parking_lot_to_cstr(obj: *mut c_void) -> *mut std::ffi::c_char {
...
#[no_mangle]
pub extern "C" fn parking_lot_getattr(obj: *mut c_void, name: *const std::ffi::c_char) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn parking_lot_call(
    _handle: *mut c_void,
...
#[no_mangle]
pub extern "C" fn parking_lot_call1(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
    a: *mut c_void,
) -> *mut c_void {
...
#[no_mangle]
pub extern "C" fn parking_lot_call2(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn parking_lot_call3(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn parking_lot_call4(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn parking_lot_call5(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn parking_lot_call6(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn parking_lot_call_kw(
    _handle: *mut c_void,
    fn_name: *const std::ffi::c_char,
    args: *mut c_void,
    kwargs: *mut c_void,
...
#[no_mangle]
pub extern "C" fn parking_lot_construct(
...
#[no_mangle]
pub extern "C" fn parking_lot_call_method(
    handle: *mut c_void,
    type_name: *const std::ffi::c_char,
...
#[no_mangle]
pub extern "C" fn parking_lot_free_type(handle: *mut c_void, type_name: *const std::ffi::c_char) {
...
#[no_mangle]
pub extern "C" fn parking_lot_last_error() -> *mut std::ffi::c_char {
    LAST_ERROR.with(|e| {
